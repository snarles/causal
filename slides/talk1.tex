...
\author[Zheng and Zhao]{Charles Zheng and Qingyuan Zhao}
\institute[Stanford]
{Stanford University}
\date{\today}
...
\begin{frame}
  ...
  \end{itemize}
  These three intuitive principles are equivalent to \emph{factorization} definition of graphical model:
  \[
  p(x_1,\hdots, x_k) = \prod_{i=1}^k p(x_i|\text{Parents}(x_i)).
  \]
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
  \item Suppose we want to reduce the expression level of PKC in the
    cell.  We have an enzyme that activates PIP2 and another enzyme that
    inhibits PIP2. Which one should we use, if at all?
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  First, we need to define \emph{causal effect}. Three essentially equivalent ways:
  \begin{itemize}
  \item The graphical approach via Bayesian network.
    \[
    \mathrm{E}[PKC | do(PIP2)]
    \]
    \vspace{-1.5em}
    \begin{itemize}
    \item Thomas Bayes (1763), Sewell Wright (1920s), Judea Pearl (1980s--1990s).
    \end{itemize}
  \item The functional approach: Structural Equation Models.
    \[
    Y = f(\mathrm{parents}(Y);\epsilon_Y)
    \]
    \vspace{-1.5em}
    \begin{itemize}
    \item Sewell Wright's path analysis (1920s), Karl J\"{o}reskog and Dag
      S\"{o}rbom's LISREL (1970s), confirmatory factor analysis,
      ``invariance principle''.
    \end{itemize}
  \item The potential outcome appraoch:
    \[
    \mathrm{E}[PKC_{PIP2+} - PKC_{PIP2-}]
    \]
    \vspace{-1.5em}
    \begin{itemize}
    \item Neyman (1923), Rubin (1970s).
    \end{itemize}
  \end{itemize}
...
\begin{frame}
  \frametitle{Three \emph{Causal} Questions}
  \begin{itemize}
...
\begin{frame}
  \frametitle{Example: observational study}
  \begin{center}
  ...
  \end{center}
  \begin{itemize}
  \item $X = \{0, 1\}$ treatment variable, $Y$ outcome of interest, $Z$ confounders.
  \item Want to estimate average treatment effect.
...
\begin{frame}
  \frametitle{Adjusting for confounders: the graphical approach}
  It suffices to estimate $\mathrm{P}(y|do(x))$.
  \begin{itemize}
  \item Factorization: $\mathrm{P}(x, y, z) = \mathrm{P}(y|x,z)\mathrm{P}(x|z)\mathrm{P}(z).$
  \item Step one: convert between `do' notation and probability
    \[
    \mathrm{P}(y | x, z) = \mathrm{P}(y | do(x), z).
    \]
  \item Step two: applying law of total probability gives the \emph{backdoor formula}
    \[
    \begin{split}
      \mathrm{P}(y|do(x)) &= \sum_z \mathrm{P}(y, z|do(x)) = \sum_z \mathrm{P}(y|do(x), z)
      \mathrm{P}(z|do(x))\\ &= \sum_z \mathrm{P}(y|x, z) \mathrm{P}(z).
    \end{split}
    \]
  \item Step three: plug in the empirical $\hat{\mathrm{P}}(y|x, z)$ and $\hat{\mathrm{P}}(z)$.
  \end{itemize}
\end{frame}

\begin{frame}
  \frametitle{Adjusting for confounders: the potential outcome approach}
  It suffices to estimate $\mathrm{P}(x|z)$, because
  \[
  \begin{split}
    \mathrm{P}(y|do(x)) &= \sum_z \mathrm{P}(y|x, z) \mathrm{P}(z) = \sum_z
    \frac{\mathrm{P}(x, y, z)}{\mathrm{P}(x,z)} \mathrm{P}(z) \\
    &= \sum_z \frac{\mathrm{P}(x, y, z)}{\mathrm{P}(x|z)}.
  \end{split}
  \]
  \begin{itemize}
  \item $\mathrm{P}(x|z)$ is called the \emph{propensity score}
    (Rosenbaum and Rubin, 1983).
  \item $\mathrm{P}(x|z)^{-1}$ is called the \emph{inverse probability
    weight}.
  \item Coarsened version: matching, subclassification.
  \item This approach is more appealing to statisticians and was
    derived before Judea Pearl's \emph{do} calculus.
  \end{itemize}
\end{frame}


\begin{frame}
  \frametitle{Adjusting for confounders: the functional approach}
  It ``sufficies'' to estimate $\mathrm{P}(y|x,z)$, because
  $\mathrm{P}(y|x,z) = \mathrm{P}(y|do(x),z)$.
  \begin{itemize}
  \item The quantity of interest is changed from $\mathrm{P}(y|do(x))$
    to $\mathrm{P}(y|do(x),z)$, but that's perhaps even better!
  \item In practice, we just run an \emph{outcome regression}.
  \item The same idea appears in
    \begin{itemize}
    \item Covariance adjustment in randomized experiment (Fisher,
      1935).
    \item Doubly robust estimation (Robins, 1990s).
    \item The invariance principle (Peters, B\"{u}hlmann and Meinshausen, 2015).
    \end{itemize}
...
\begin{frame}
  ...
  In part II of the talk, we'll go into more detail about inference
  with finite sample, limitations and criticism, with a focus on the
  invariance approach by Peters, B\"{u}hlmann and Meinshausen. Could
  this new approach extend the applicability of causal inference?
  % \begin{itemize}
